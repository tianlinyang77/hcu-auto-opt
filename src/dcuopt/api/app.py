...
from dcuopt.domain.errors import (
    Conflict,
    ContractError,
    NotFound,
    StaleClaimToken,
...
def create_app(
    ...
    async def lifespan(application: FastAPI):
        repo = repository or PostgresRepository(
            os.getenv("DCUOPT_DATABASE_URL", DEFAULT_DATABASE_URL)
        )
        application.state.repository = repo
        if os.getenv("DCUOPT_AUTO_MIGRATE", "true").lower() == "true":
            repo.migrate()
        yield

    application = FastAPI(
        title="DCU Auto Opt Control Plane",
