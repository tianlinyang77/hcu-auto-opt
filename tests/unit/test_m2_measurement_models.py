...
def test_round_measurement_ref_enforces_search_holdout_discriminator() -> None:
    search = _reference(0)
    holdout = _reference(1, phase=RoundPhase.HOLDOUT)
    ...
    with pytest.raises(ValidationError, match="cannot bind Holdout"):
        _reference(2, holdout_family_hash=_hash("c"))
...
def test_round_measurement_ref_rejects_synthetic_verdict_and_naive_timestamp() -> None:
    with pytest.raises(ValidationError, match="Input should be False"):
        _reference(0, synthetic=True)
    with pytest.raises(ValidationError, match="Extra inputs are not permitted"):
        _reference(0, verdict="faster")
    with pytest.raises(ValidationError, match="timezone-aware"):
        _reference(0, created_at=datetime(2026, 8, 27))
...
def test_measurement_authority_rejects_identity_reuse(field_name: str) -> None:
    first = _reference(0)
    second = _reference(
        1,
        phase=RoundPhase.HOLDOUT,
...
def test_measurement_authority_rejects_member_phase_and_family_drift() -> None:
    first = _reference(0)
    duplicate_member_phase = _reference(1, candidate_id=first.candidate_id)
    family_drift = _reference(2, candidate_family_hash=_hash("e"))
...
def test_measurement_authority_returns_deterministic_phase_candidate_order() -> None:
    search = _reference(0)
    holdout = _reference(1, phase=RoundPhase.HOLDOUT)

    ordered = validate_round_measurement_refs((search, holdout))

    assert ordered == (holdout, search)
