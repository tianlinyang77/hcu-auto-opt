...
| BusinessCandidatePackageStoreDescriptor | C | A/D | 固定同一可信 Store 中的 2～4 个真实源码包、Baseline 和允许替换路径 |
| BusinessCandidateFamilyVerificationRecord | C Verifier | A/D | Store 重读和 Baseline 源码重放均通过；只表示可申请正式测试窗口，不表示正确或更快 |
