...
| FormalEvidenceAcceptanceSnapshot / Review v2 | D deployment | #102 readiness / project owner | 调用方只给 Round 与 audit 身份；D 从 production Root 重读 Context、Round、Barrier、Reveal/FWER、EvidenceBundle 和 Signoff，复用既有 Finalizer 递归重建并做 allowlisted 验签；只有全链通过才签发 `accepted_for_formal_window`，仍不授权窗口或自动发布 |
