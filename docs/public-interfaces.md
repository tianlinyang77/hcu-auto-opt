...
- `BusinessCandidatePackageStoreDescriptor`：固定 C 线可信 Store、锁定 Baseline、允许替换路径和
  2～4 个既有 `CandidateSourcePackageRef`；它是 Store 的部署描述，不是新的 Candidate 格式；
- `BusinessCandidateFamilyVerificationRecord`：记录 Store 重读、完整 Baseline 源码重放和
  Candidate Source Hash 复算结果，并携带 Family 固定的测试任务；记录固定为不访问 HCU、
  未测量性能且不能自动发布。
